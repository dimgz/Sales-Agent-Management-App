...
import output.ReportCreationHTML;
import output.ReportCreationTXT;
import output.ReportCreationXML;
...
public class ReportWindow extends JDialog {
	...
	private AgentOperationsWindow selectionWindow;
	...
	public ReportWindow(final AgentOperationsWindow sw, AgentSalesAccount agent,double tSales,int tItems,
	...
	private void initialise(){
		...
		resultWindowPanel.setLayout(null);
		{
			...
			xmlReportButton.setBounds(436, 143, 163, 84);
		...
		htmlReportButton.setBounds(436, 252, 163, 84);
	...
	private void outputTXTButtonPressed(ActionEvent evt) throws IOException 
	{
		try
		{
			...
			TXTFileChooser.showSaveDialog(null);
			File reportFileTXT = TXTFileChooser.getSelectedFile();
			
			ReportCreationTXT makeTXTFile = new ReportCreationTXT(selectedAgent, reportFileTXT);
			makeTXTFile.saveFile();
			
			JOptionPane.showMessageDialog(null,"Η αναφορά αποθηκέυτηκε επιτυχώς!");	
		}
	...
	private void outputHTMLButtonPressed(ActionEvent evt) throws IOException 
	{
		try
		{
			...
			HTMLFileChooser.showSaveDialog(null);
			File reportFileHTML = HTMLFileChooser.getSelectedFile();
			
			ReportCreationHTML makeHTMLFile = new ReportCreationHTML(selectedAgent, reportFileHTML);
	...
	private void outputXMLButtonPressed(ActionEvent evt) throws IOException 
	{
		try
		{
			...
			XMLFileChooser.showSaveDialog(null);
			File reportFileXML = XMLFileChooser.getSelectedFile();
			
			ReportCreationXML makeXMLFile = new ReportCreationXML(selectedAgent, reportFileXML);
