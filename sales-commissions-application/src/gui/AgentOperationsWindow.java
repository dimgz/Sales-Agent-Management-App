...
public class AgentOperationsWindow extends JDialog {
	...
	private AgentInputWindow inputDialog;
	private AgentSalesAccount selectedAgent;
	@SuppressWarnings("unused")
	private String fileType;
	
	public AgentOperationsWindow(AgentInputWindow dialog, AgentSalesAccount agent, String fileTypeFlag) {
		inputDialog = dialog;
		selectedAgent = agent;
	...
	public void initialise(){
		...
		try{
			...
			JOptionPane.showMessageDialog(null,"Υπήρξε κάποιο πρόβλημα προσπαθήστε ξανά!");
	...
	{	
		 totalSales = getValueIfCheckBoxSelected(totalSalesCheckBox, (float) selectedAgent.calculateTotalSales());
		 totalItems = (int) getValueIfCheckBoxSelected(totalItemsCheckBox, selectedAgent.calculateTotalItems());
		 commission = getValueIfCheckBoxSelected(commissionCheckBox, (float) selectedAgent.calculateCommission());
		 shirtSales = getValueIfRadioSelected(shirtRadio, selectedAgent.calculateSpecificKindSales("Shirts"));
		 skirtSales = getValueIfRadioSelected(skirtRadio, selectedAgent.calculateSpecificKindSales("Skirts"));
		 coatsSales = getValueIfRadioSelected(coatRadio, selectedAgent.calculateSpecificKindSales("Coats"));
		 trousersSales = getValueIfRadioSelected(trousersRadio, selectedAgent.calculateSpecificKindSales("Trousers"));
		 
		ReportWindow rs = new ReportWindow(this,selectedAgent, totalSales, totalItems, shirtSales, skirtSales, trousersSales, coatsSales, commission);
		rs.setVisible(true);
		this.setVisible(false);		
	}
	
	private float getValueIfCheckBoxSelected(JCheckBox checkBox, float calculation)
	{
	...
	private float getValueIfRadioSelected(JRadioButton radioButton, float calculation)
	{
	...
	private void addReceiptButtonPressed(ActionEvent evt) throws IOException {
	...
	private void appendFile() throws IOException{
		ArrayList<Receipt> agentReceipts = selectedAgent.getReceipts();
		Receipt lastReceipt = agentReceipts.get(agentReceipts.size()-1);
		selectedAgent.getFileAppender().setReceipt(lastReceipt);
		selectedAgent.getFileAppender().appendReceipt();
	}
	...
	private void addReceipt()
	{
		...
		if(isSpecificKind(kindTextField.getText())) 
		{
		...
		{
			JOptionPane.showMessageDialog(null,"Δεν συμπληρώσατε σωστά όλα τα πεδία.Προσπαθείστε ξανά!");
		}
	}
